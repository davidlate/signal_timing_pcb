...
static void i2s_play_task(void * user_ctx){     
    ...
    buff_pos = 0;
    while (buff_pos < buff_len && audio_pos < audio_len){            //fill up the buffer.  Volume is written now, so there is a delay between when it is adjusted and when it is played.
